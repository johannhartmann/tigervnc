...
// IDXGIOutput1::DuplicateOutput is the API we depend on; it lives in
// dxgi1_2.h (Windows 8 / Server 2012). Below the modern target the
// declaration isn't available, so initialize() falls through to the
// scaffold path that reports NotSupported.
#if defined(_WIN32_WINNT) && _WIN32_WINNT >= 0x0602
#  define TIGERVNC_DXGI_IMPL 1
#  include <d3d11.h>
#  include <dxgi1_2.h>
#else
#  define TIGERVNC_DXGI_IMPL 0
#endif
...
CaptureStatus DxgiDesktopDuplicationBackend::initialize(int /*monitorIndex*/) {
  if (!probe()) {
    vlog.info("DXGI runtime not available; backend disabled");
    return CaptureStatus::NotSupported;
  }
  vlog.info("DXGI Desktop Duplication body requires _WIN32_WINNT >= 0x0602 "
            "(Windows 8 / TIGERVNC_WINDOWS_TARGET=modern); current build "
            "is at the legacy profile, scaffold only -- factory will fall "
            "back");
  return CaptureStatus::NotSupported;
}
